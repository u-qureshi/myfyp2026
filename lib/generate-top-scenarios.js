...
import { getSubmittedAvailabilityMap } from '@/lib/faculty-portal-store'
...
export async function prepareTimetableInput(departmentId, semester, departmentCode = null) {
  ...
  const availabilityMap = await getSubmittedAvailabilityMap(departmentId, sem)
  const facultyWithAvailability = facultyData.filter((f) => availabilityMap[f.id])
  if (!facultyWithAvailability.length) {
    throw new Error(
      'No faculty have submitted availability for this department and semester. Request availability from faculty first.'
    )
  }

  const mappedFaculty = mapFacultyForGenerator(
